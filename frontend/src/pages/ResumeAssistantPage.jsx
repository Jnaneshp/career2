...
const BACKEND_URL = process.env.REACT_AI_BACKEND_URL || 'https://backend-resume-gdgh.onrender.com/api';
...
export default function AIAssistantPage({ currentUser }) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // POST to backend Groq-powered endpoint
      ...
      const aiMessage = {
        role: 'assistant',
        content: response.data?.response || 'Sorry, no response was returned by the AI assistant.',
  ...
  // Resume upload + AI advice
  const handleResumeSelection = (e) => {
    setSelectedResume(e.target.files[0]);
  };

  const analyzeResume = async () => {
    if (!selectedResume || !currentUser?.id) return;
    setResumeAdvice('');
    ...
    try {
      ...
      setResumeAdvice(response.data?.recommendations || 'No advice received.');
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <div className="container mx-auto px-4 py-8 h-[calc(100vh-88px)]">
        <div className="max-w-4xl mx-auto h-full flex flex-col">
          ...
          <Card className="mb-6 p-4 bg-white/90">
...
{resumeAdvice && (
  <div className="mt-6 bg-gradient-to-br from-purple-50 to-blue-50 border border-purple-200 rounded-xl p-4">
    <h4 className="font-bold mb-2 text-purple-700">AI Career Advice</h4>
    <div className="text-sm markdown-content text-gray-800">
      <ReactMarkdown>{resumeAdvice}</ReactMarkdown>
    </div>
  </div>
)}
          </Card>
              ...
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center" data-testid="empty-chat-state">
                  ...
                  <p className="text-gray-600 mb-6 max-w-md">
                    Ask me anything about resumes, career paths, interview prep, or skill development!
                  </p>
